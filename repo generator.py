...
# Comment out this line if you have .pyc or .pyo files you need to keep
    ...
    def Create_Zips(self,addon_id,version):
        ...
        addon_folder = os.path.join('_zips',addon_id)
        if not os.path.exists(addon_folder):
            os.makedirs(addon_folder)

        final_zip = os.path.join('_zips',addon_id,'%s-%s.zip' % (addon_id,version))
...
# loop thru and add each addons addon.xml file
        for addon in addons:
            try:
                if (not os.path.isdir(addon) or addon == "_zips" or addon.startswith('.')): continue
...
# clean and add closing tag
        ...
        self._save_file(addons_xml.encode( "utf-8" ), file=os.path.join('_zips','addons.xml'))

    def _generate_md5_file(self):
        try:
            m = md5.new(open(os.path.join('_zips','addons.xml')).read()).hexdigest()
            self._save_file(m, file=os.path.join('_zips','addons.xml.md5'))
        except Exception, e:
